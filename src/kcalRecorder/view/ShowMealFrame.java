...
public class ShowMealFrame extends JFrame {
	...
	public ShowMealFrame(String title, ArrayList<Food> foodList, ArrayList<Foods> foodsList, ArrayList<Meal> mealList) {
		super(title);
	...
	public class showFoodMainPanel extends JPanel {
		...
		public void makeUI() {
			...
			foodTextAreaWithScroll = new JScrollPane(foodListTextArea, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
	...
	public void updateFoodListTextArea() {
		...
		for (int i = 0; i < mealList.size(); ++i) {
			...
			for (int j = 0; j < foodsList.size(); ++j) {
				Foods foods = foodsList.get(j);
				if (foods.getmNo() == mNo) {
					for (int k = 0; k < foodList.size(); ++k) {
						Food food = foodList.get(k);
						if(foods.getfNo() == food.getF_no()) {
						sb.append("음식 이름 :");
						sb.append(food.getName());
						sb.append('\n');

						sb.append("음식 1인분 당 칼로리:");
						sb.append(food.getKcalPerOneHundred());
						sb.append('\n');

						sb.append("몇 인분:");
						sb.append(foods.getAmount());
						sb.append('\n');
						sb.append('\n');
						sb.append('\n');
						totalKcal += food.getKcalPerOneHundred() * foods.getAmount();
						}
					}
				}
	...
	public void updateFoodListTextAreaByController(ArrayList<Meal> mealList2, ArrayList<Foods> foodsList2,
			ArrayList<Food> foodList2) {
		StringBuilder sb = new StringBuilder();
		SimpleDateFormat sdf = new SimpleDateFormat("YYYY년MM월dd일 hh:mm");
		foodListTextArea.setText("");
		for (int i = 0; i < mealList2.size(); ++i) {
			Meal meal = mealList2.get(i);
			int mNo = meal.getmNo();
			Date date = meal.getDate();
			int totalKcal = 0;
			sb.append(sdf.format(date));
			sb.append('\n');
			for (int j = 0; j < foodsList2.size(); ++j) {
				Foods foods = foodsList2.get(j);
				if (foods.getmNo() == mNo) {
					for (int k = 0; k < foodList2.size(); ++k) {
						Food food = foodList2.get(k);
						if(foods.getfNo()==food.getF_no()) {
						sb.append("음식 이름 :");
						sb.append(food.getName());
						sb.append('\n');
